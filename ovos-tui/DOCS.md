...
**Fixed properly upstream in `ovos-tui-client` v0.1.25**, which added `--web-public-url`: a separate flag that overrides only the URLs baked into the page, completely independent of the bind address. This add-on now always binds `0.0.0.0` (works unconditionally) and passes `web_public_url` through to `--web-public-url` -- leave it blank to default to this add-on's own hostname (works wherever mDNS does, same as before), or set it explicitly (e.g. `http://192.168.1.50:8000`, using this add-on's own LAN-reachable address and the port shown in this add-on's Network settings) if that doesn't resolve for your browser.

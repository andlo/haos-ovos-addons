...
**A separate, confirmed root cause for individual Adapt intents silently never matching: a capitalization mismatch inside `ovos_workshop` itself, not a bug in any specific skill.** Found via a live bus capture (using `ovos-busmon`) of `ovos-skill-alerts`' own boot: its `IntentBuilder("CreateAlarm").require("alarm").require("create")` sends a `register_intent` requiring `ovos_skill_alerts_openvoiceosalarm`/`...create` (lowercase, from `intents.py`'s own `munge_intent_parser`, which does not title-case), while its `alarm.voc`/`create.voc` resource files get loaded and registered as `ovos_skill_alerts_openvoiceosAlarm`/`...Create` (title-cased, from `resource_files.py`'s own `file_name[:-4].title()`). The two never match, so this specific intent can never fire — with no error anywhere, since bus registration itself always "succeeds." Confirmed this is a real `ovos_workshop` inconsistency, not specific to this one skill: any skill following the same standard convention (a lowercase `.require("x")` matched by a same-named, lowercase `x.voc` file) hits it. Reported upstream: [OpenVoiceOS/ovos-workshop#559](https://github.com/OpenVoiceOS/ovos-workshop/issues/559). This likely explains at least some of the general Adapt-matching unreliability this stack has shown — a skill's Padatious-trained phrasings for the same underlying intent keep working (that registration path doesn't go through this same namespacing code), masking the problem unless a specifically Adapt-only phrasing is tried (e.g. "set an alarm" with no time, which only `CreateAlarm`'s own Adapt intent — not any of `ovos-skill-alerts`' own Padatious training data — is meant to catch).

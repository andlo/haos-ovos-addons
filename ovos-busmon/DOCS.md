...
- Installed from the git repo directly, not PyPI -- confirmed by testing on real hardware that the published `0.1.0` wheel is missing its own `static/` directory (the web UI's `index.html`/JS/CSS) entirely, so `GET /` 404s even though the FastAPI service itself starts fine and genuinely connects to the bus (`GET /api/status` responds correctly). A packaging bug, not a config issue on this add-on's side -- reported upstream: [OpenVoiceOS/ovos-busmon#65](https://github.com/OpenVoiceOS/ovos-busmon/issues/65). Revisit pinning to a PyPI release once that's fixed and published.
